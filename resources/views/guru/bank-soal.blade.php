...
@section('content')
@php
    // Form mana yang barusan disubmit, dipakai untuk membuka ulang modal yang
    // relevan dan menampilkan error di dalamnya kalau validasi gagal —
...
<script>
    (function () {
        // Peta mapel -> kelas yang benar-benar diajarkan guru ini. Kosong
        // berarti tidak ada pembatasan (mis. akun guru tanpa profil guru).
        const subjectClassroomMap = @json($subjectClassroomMap ?? []);
        const subjectSelect = document.getElementById('kuisSubjectSelect');
        const classroomSelect = document.getElementById('kuisClassroomSelect');
        if (! subjectSelect || ! classroomSelect || Object.keys(subjectClassroomMap).length === 0) {
            return;
        }
        ...
        subjectSelect.addEventListener('change', function () {
            const allowedIds = (subjectClassroomMap[this.value] || []).map(String);
            classroomSelect.innerHTML = '';
            classroomSelect.appendChild(new Option('Pilih', ''));
            allOptions.filter((option) => allowedIds.includes(option.value)).forEach((option) => {
    ...
    // pesan errornya dan isian yang sudah diketik guru tetap terlihat,
    // bukan malah hilang begitu saja setelah halaman reload.
