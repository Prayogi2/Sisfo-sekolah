...
<div class="container-fluid">
    <div class="mb-4"><h1 class="h3 mb-1 fw-bold">Kelola Akun Login</h1><p class="text-muted mb-0">Reset password guru, wali murid, atau siswa.</p></div>
    ...
    <div class="card border-0 shadow-sm"><div class="table-responsive"><table class="table table-hover align-middle mb-0">
        ...
        @forelse($users as $user)
            <tr><td class="fw-semibold">{{ $user->name }}</td><td>{{ ucfirst($user->role) }}</td><td>{{ $user->email }}</td><td>{{ in_array($user->role, ['wali', 'siswa'], true) ? $user->name : $user->email }}</td><td class="text-end">
